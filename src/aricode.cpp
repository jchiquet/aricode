...
List cpp_SortPairs(IntegerVector c1, IntegerVector c2) {
  ...
  return List::create(
      Named("pair_nb") = pair_count[ind], Named("pair_c1") = pair_c1[ind],
      Named("pair_c2") = pair_c2[ind], Named("c1_nb") = count1[count1 > 0],
  ...
  for (int i = 0; i < ni_.size(); i++) {
    for (int j = 0; j < n_j.size(); j++) {
      ...
      for (int nij = start_nij; nij <= end_nij; nij++) {
        double t1 = ((float)nij / (float)N) *
                    std::log((float)(nij * N) / (float)(ni_[i] * n_j[j]));

        double t2 = std::exp((ni_f[i] + nj_f[j] + Nmni_f[i] + Nmnj_f[j] - N_f -
                              lgamma(1 + nij) - lgamma(1 + ni_[i] - nij) -
                              lgamma(1 + n_j[j] - nij) -
                              lgamma(1 + N - ni_[i] - n_j[j] + nij)));
...
List getRank(IntegerVector classi) {
  ...
  // output as a list
  return List::create(Named("index") = index, Named("translator") = translator,
                      Named("translated") = translated);
}
