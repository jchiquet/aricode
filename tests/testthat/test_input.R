...
test_that("Testing supposedly handled input types", {
  ...
  ref.object <-
    list(
      spMat = NULL,
      levels = list(c1 = unique(c1.numeric), c2 = unique(c2.numeric)),
      nij = rep(3, 3), ni. = rep(3, 3), n.j = rep(3, 3),
      pair_c1 = 0:2, pair_c2 = 0:2
    )
  expect_that(sort_pairs(c1.numeric, c2.numeric), equals(ref.object))
  ...
  expect_that(sort_pairs(c1.integer, c2.integer), equals(ref.object))
  ...
  expect_that(sort_pairs(c1.char, c2.char), equals(ref.object))
  ...
  expect_that(sort_pairs(c1.factor, c2.factor), equals(ref.object))
  ...
  expect_that(sort_pairs(c1.char, c2.factor), equals(ref.object))
})
...
test_that("Testing error for not handled input types", {
  ...
  c2NA <- c2
  c2NA[1] <- NA
  expect_error(sort_pairs(c1, c2NA))

  cat("\n-different sizes-")
  expect_error(sort_pairs(c1, c2[-1]))

  cat("\n-list type-")
  expect_error(sort_pairs(as.list(c1), as.list(c2)))

  cat("\n-matrix type-")
  expect_error(sort_pairs(as.matrix(c1), as.matrix(c2)))

  cat("\n-data.frame type-")
  expect_error(sort_pairs(as.data.frame(c1), as.data.frame(c2)))
})
